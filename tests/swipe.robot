...
Deve poder remover o Darth Vader
    ...
    ${name}    Set Variable    Darth Vader

    Start session
    Get Started

    Navigate to    Star Wars
    Go to item     Lista    ${name} 

    ${positions}    Get Element Location    xpath=//android.widget.TextView[@text="${name}"]/../../..//*[contains(@resource-id, "indicator")]
